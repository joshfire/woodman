...
define(function (require) {
   ...
   *  filter types.
   */
  var createFilter = function (rawFiltersConfig) {
    var filtersConfig = [];
    var filters = [];
    var filter = null;
  ...
  LoggerContext.prototype.initialize = function (config) {
    ...
    this.filter = createFilter(config.filters);
    ...
    utils.each(appendersConfig, function (appenderConfig) {
      ...
      if (appenderConfig.filters) {
        appenderConfig.filter = createFilter(appenderConfig.filters);
      }
      else if (appenderConfig.filter) {
        appenderConfig.filter = createFilter([appenderConfig.filter]);
      }
    ...
    utils.each(loggersConfig, function (loggerConfig) {
      ...
      if (loggerConfig.filters) {
        loggerConfig.filter = createFilter(loggerConfig.filters);
      }
      else if (loggerConfig.filter) {
        loggerConfig.filter = createFilter(loggerConfig.filter);
      }
