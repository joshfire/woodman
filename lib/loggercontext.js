...
define([
  ...
  './loglevel',
  './utils'
], function (Logger, Appender, Layout, LogLevel, utils) {
  ...
  LoggerContext.prototype.initialize = function (config) {
    ...
    utils.each(appendersConfig, function (appenderConfig) {
      ...
      appenderConfig.layout = new layoutClass(layoutConfig);
  ...
  LoggerContext.prototype.reset = function () {
    ...
    this.rootLogger.level = LogLevel.all;
  };
  ...
  LoggerContext.prototype.propagateLevels = function () {
    if (this.rootLogger.level === LogLevel.inherit) {
      this.rootLogger.level = LogLevel.all;
    }
    utils.each(this.loggers, function (logger) {
      var parent = logger;
      while (parent.level === LogLevel.inherit) {
