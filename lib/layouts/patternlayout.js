 ...
 *
 * The implementation also supports the "%domain" pattern to output the ID
 * of the current domain in a Node.js application that uses domains.
 *
...
define(function (require) {
   ...
   */
  PatternLayout.prototype.toMessageBits = function (evt, options) {
    return this.formatLogEvent(evt, this.pattern, options);
  };
   ...
   * @param {string} options Serialization options. The only supported option
   *   is "preserveObjects" to preserve the objects as-is
   * @return {Array<object|string>} The event formatted according to the
   *  pattern. The function returns an array with only one item when the
   *  "preserveObjects" option flag is not set. It returns an array with as
   *  many bits as needed when the "preserveObjects" option flag is set.
   */
  PatternLayout.prototype.formatLogEvent = function (evt, pattern, options) {
    var formattedBits = [];
    var formattedString = '';
    var rawResult = [];
    var result = {
     ...
     */
    var rPattern = /%(-?[0-9]+)?(\.?[0-9]+)?(logger|date|domain|highlight|message|level|relative|[cdhmnpr%])(?:\{([^\}]+)\})?|([^%]+)/g;
    ...
    while ((rawResult = rPattern.exec(pattern))) {
      ...
      case 'domain':
        // Add the ID of the current Node.js domain if known, "global" if
        // the code runs in the global domain, "anonymous" if the domain exists
        // but does not have an ID, and "unknown" if the code does not run in a
        // Node.js environment.
        // The parameter gives the name of the property that contains the ID of
        // the domain. It may contain dots to have the code go down to nested
        // levels. The code uses "id" if the parameter is not specified.
        // The "domain" pattern is an extension to log4j, useful e.g. to track
        // traces that relate to the same HTTP request in an HTTP server,
        // provided each request gets treated in a dedicated domain.
        str = result.params || 'id';
        if (typeof process !== 'undefined') {
          if (process.domain) {
            str = str.split('.');
            replacement = process.domain;
            while (replacement && (str.length > 0)) {
              replacement = replacement[str.shift()];
            }
            replacement = replacement || 'anonymous';
          }
          else {
            replacement = 'global';
          }
        ...
        if (utils.isFunction(replacement)) {
      ...
      case 'm':
        ...
        if (options.preserveObjects) {
          replacement = evt.getMessage().getFormattedParams({
      ...
      if (utils.isArray(replacement)) {
        // Replacement is an array when options.preserveObjects is set and
        // when the current bit is the serialization of the message
        if (formattedString) {
          // Concatenate the first bit if it is a string because the
          // console would add a non desired string otherwise
          if (utils.isString(replacement[0])) {
            formattedString += replacement[0];
            replacement = replacement.slice(1);
          }
          formattedBits.push(formattedString);
          formattedString = '';
        }
        if (replacement.length > 0) {
          formattedBits = formattedBits.concat(replacement);
        }
      }
      else {
