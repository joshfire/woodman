...
define([
  ...
  '../loglevel',
  '../simpledateformat'
], function (Layout, LogLevel, SimpleDateFormat) {
  ...
  /**
   * Log level to name conversion table, built from the LogLevel enumeration
   */
  var level2Name = (function () {
    var table = {};
    var level = null;
    for (level in LogLevel) {
      if (LogLevel.hasOwnProperty(level)) {
        table[LogLevel[level]] = level;
      }
    }
    return table;
  })();
  ...
  PatternLayout.DATE_DATEFORMAT = 'dd MMM yyyy HH:mm:ss,SSS';
  PatternLayout.ABSOLUTE_DATEFORMAT = 'HH:mm:ss,SSS';
  ...
  PatternLayout.TTCC_CONVERSION_PATTERN = '%r %p %c %x - %m%n';
    ...
    while ((rawResult = rPattern.exec(this.pattern))) {
      ...
      switch (result.pattern) {
        ...
        case "p":
          ...
          replacement = level2Name[evt.getLevel()] || '' + evt.getLevel();
          break;
